...
1. [Visão Geral e Introdução](#1-visão-geral-e-introdução)
...
6. [Requisitos de Interface Externa](#6-requisitos-de-interface-externa)
7. [Anexos e Rastreabilidade](#7-anexos-e-rastreabilidade)
...
- Processamento de pagamentos online (gateway de cartão, PIX, boleto) e cobrança automatizada. O sistema registra apenas a confirmação da entrada do pedido (RN021), sem apurar valores, emitir cobrança ou controlar recebimentos.
...
| Tira | Componente superior do chinelo que prende o pé ao solado; possui cor, numeração e modelo ( slim ou tradicional grossa) definidos no pedido. |
| Arte | Imagem, layout ou personalização gráfica aplicada ao solado. |
| Sublimação | Processo de transferência da arte para o material por ação de calor e pressão, a partir de papel sublimático impresso com tinta sublimatica. |
| Papel sublimático | Insumo de impressão utilizado na sublimação; consumido à razão de 1,5 unidade por par. |
| Adicional | Item complementar opcional agregado ao pedido (ex.: Super personalização (nome diferente em cada par) - R$1,50 por par,-Ponto de luz - R$1,20 cada (disponíveis em dourado e prateado), jóia de chinelo (escrito: noiva, mãe, madrinha, filha, love ou MEUS15 disponível em dourado ou prateado) - R$10,00 e a produção de Arte exclusiva (produzida pela nossa equipe de arte)no valor de *R$ 70,00 |
| Embalagem | Material de acondicionamento do produto final, definido por pedido. Opções de embalagem são: Saquinho com fitilho - R$1, 20 cada (entregue já embalado), Fita de cetim (unindo os dois pares em um laço de cetim em cor escolhida pelo cliente) - R$2,00 cada ,Saquinho de organza branco perolado (entregue já embalado) - R$3,80 cada, Saquinhos Personalizados - R$8,90 cada, Ecobag Personalizados R$10,90 cada, Necessaire -$13,00 cada e se desejar adicionar nome diferente a cada necessaire/saquinho cobramos o adicional de Super personalização (R$1,50 a mais por unidade) |
| Insumo | Todo material consumido na produção: placa, tecido, papel sublimático, tira, adicional e embalagem. |
| Evento | Ocasião a que se destina o pedido personalizado (ex.: casamento, formatura, aniversário ou evento promocional), com data e características próprias. |
...
- **Sistema independente:** concentra clientes, pedidos, insumos, estoque e produção em uma única base de dados, garantindo integridade referencial e fonte única da verdade e situação em tempo real.
- **Substituição de controles legados:** substitui as planilhas de pedidos, as fichas de produção manual e de anotação de numeração e os cálculos manuais de consumo de material.
- **Fronteira com sistemas de terceiros:** a emissão fiscal e a gestão financeira permanecem em ferramentas externas; o sistema apenas disponibiliza a exportação de dados de pedidos para alimentá-las, sem integração automatizada na versão 1.0 mas com possibilidade **futura** de integração a um sistema de emissão de nota fiscal e gestão de entrada e saída de notas para gestão fiscal.
...
| Produção                | Responsável pela execução e pelo acompanhamento dos pedidos liberados no chão de fábrica.                 | Leitura de pedidos liberados e de suas fichas de produção; atualização do status produtivo; registro de finalização e de eventuais perdas/refugos. Não cria nem edita pedidos, clientes ou preços.                                                                   |
...
- **Periféricos:** impressora a laser ou jato de tinta pra a emissão da ficha de produção e das etiquetas de pedido; leitor de código de barras/QR Code (opcional). para apontamento de produção.
...
- As artes serão: Arte de catalogo (alterada para cada pedido), arte produzidas internamente pela equipe de arte (arte exclusiva) ou fornecidas pelo cliente todas em formato digital, e apenas o arquivo aprovado será vinculado ao pedido.
...
|------------|------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|
| RF001      | Autenticação de Usuário                  | Permitir login por senha, com suporte a MFA para o perfil Administrador.                                                                                                             | Alta           |
...
| RF004      | Cadastro de Evento                       | Registrar as informações do evento vinculado ao pedido (tipo de evento, data do evento e nome dos homenageados como: nome os noivos, aniversariantes, formandos ou empresa ).        | Alta           |
...
| RF007      | Distribuição por Gênero Numeração e cor. | Registrar a grade do pedido, distribuindo os pares por gênero (masculino, feminino, infantil) por numeração do 17/18 ao 45/46 e por cor ( lista de cores possíveis para cada gênero. | Alta           |
| RF008      | Definição de Tiras                       | Especificar modelo e cor das tiras por item ou por grade do pedido.                                                                                                                  | Alta           |
...
| RF015      | Liberação do Pedido para Produção        | Alterar a situação do pedido para liberado após o registro da confirmação da entrada (RN021), a aprovação da arte e a validação de estoque bem-sucedida. | Alta           |
...
| RF017      | Emissão da Ficha de Produção             | Gerar documento imprimível com: nome, data do evento, data de retirada, grade, tiras (cor e gênero), arte, adicionais, embalagem e insumos calculados. | Alta           |
...
3. O usuário aciona o comando "Entrar".
4. O sistema valida as credenciais e, para o perfil Administrador, solicita o segundo fator de autenticação.
5. O sistema cria a sessão e redireciona o usuário ao painel inicial correspondente ao seu perfil.
...
- **FA04 – Senha expirada (Administrador):** o sistema conduz obrigatoriamente à tela de redefinição de senha antes de liberar o acesso, conforme RN015.
...
- **Descrição:** o sistema deve permitir o registro detalhado do pedido personalizado, reunindo nome do cliente, evento, quantidade de pares, qualidade, grade de numeração, tiras( tipo e cor), arte, adicionais e embalagem.
...
**Fluxo principal:**
1. O usuário aciona a opção "Novo Pedido" e seleciona o cliente.
2. O usuário informa ou seleciona o evento vinculado (tipo, data e local) e o prazo de entrega desejado.
3. O usuário informa a quantidade total de pares e seleciona a qualidade do produto: Normal (50/50) ou Premium (70/30).
4. O usuário distribui os pares por gênero e por numeração, compondo a grade do pedido.
5. O sistema valida se o somatório da grade é igual à quantidade total informada (RN010).
6. O usuário define modelo, material e cor das tiras.
7. O usuário seleciona a arte no catálogo ou escolhe a opção de arte exclusiva com local para anexar o arquivo correspondente.
8. O usuário inclui os adicionais desejados e informa suas quantidades.
9. O sistema calcula automaticamente os insumos necessários (RF012) e apresenta o resumo técnico do pedido.
10. O usuário confirma o registro e o sistema grava o pedido na situação "Aguardando validação".
...
- **FA03 – Arte pendente:** o pedido pode ser gravado com a arte em situação "Em aprovação", mas não poderá ser liberado para produção enquanto não for aprovada (RN014).
...
**Fluxo principal:**
1. O sistema identifica a qualidade do pedido e recupera os índices aplicáveis (RN001 e RN002).
2. O sistema calcula a quantidade de placas necessárias arredondando para cima a divisão do total de pares pelo rendimento por placa (RN003).
3. O sistema calcula a metragem de tecido multiplicando a quantidade de placas pelo consumo unitário da qualidade correspondente (RN004).
4. O sistema calcula o consumo de papel sublimático multiplicando o total de pares por 1,5 (RN006).
5. O sistema apura a quantidade de tiras, de adicionais e de embalagens conforme a grade e as opções selecionadas (RN007).
6. O sistema calcula o consumo de tinta sublimática, considerando os pares e as embalagens sublimadas do pedido (RN008).
7. O sistema calcula a quantidade de tags e travas anel, considerando os pares e as necessaires do pedido (RN009).
8. O sistema apresenta a memória de cálculo detalhada por insumo e a persiste vinculada ao pedido.
...
- **Pós-condições:** necessidade de insumos calculada, gravada e disponível para a validação de estoque (RF013) e para a ficha de produção (RF017).
...
**Fluxo principal:**
1. O usuário aciona a validação do pedido ou o sistema a executa automaticamente ao final do registro.
2. O sistema apura, para cada insumo, o saldo disponível (saldo físico menos reservas de outros pedidos).
3. O sistema compara a necessidade com o saldo disponível, item a item.
4. Havendo suficiência para todos os itens, o sistema marca o pedido como "Validado" e o habilita à liberação para produção.
5. O sistema registra o resultado da validação, com data, hora e responsável, no histórico do pedido.

**Fluxos alternativos / exceções:**

- **FA01 – Insuficiência de insumo:** o sistema mantém o pedido na situação "Pendente de insumo", impede a liberação (RN011) e apresenta a relação dos itens em falta, com a quantidade faltante de cada um.
...
**Fluxo principal:**
1. O usuário do perfil Produção acessa a lista de pedidos liberados, ordenada por prazo de entrega.
2. O usuário seleciona o pedido e consulta a ficha de produção correspondente (RF017).
3. O usuário registra o avanço para a etapa seguinte: Impressão da arte, Sublimação, Corte, Montagem, Acabamento, Embalagem e Finalizado.
4. O sistema grava a etapa, o responsável e o instante do apontamento, e efetua a baixa dos insumos correspondentes (RF014).
5. Concluída a última etapa, o sistema altera a situação do pedido para "Finalizado" e o encaminha à expedição (RF021).

**Fluxos alternativos / exceções:**

- **FA01 – Tentativa de salto de etapa:** o sistema impede o registro de etapa fora da sequência definida (RN013) e exibe a etapa esperada.
...
| RN005      | Seleção de embalagem por pedido                                | O tipo de embalagem é determinado pela opção selecionada em cada pedido. A validação de disponibilidade da embalagem em estoque é obrigatória; havendo falta, o sistema gera a demanda de compra. |
...
| RN008      | Consumo de tinta sublimática                                   | Tinta sublimática = 1 ml por par. As embalagens com sublimação (saquinho personalizado e necessaire) consomem 1 ml adicional por unidade. |
| RN009      | Consumo de tag e trava anel                                    | Cada par consome 1 tag e 1 trava anel. Cada necessaire consome 1 tag e 1 trava anel adicionais. |
| RN010      | Consistência da grade de numeração                             | O somatório dos pares distribuídos por gênero e numeração deve ser exatamente igual à quantidade total de pares do pedido; divergências impedem a gravação.                |
| RN011      | Notificação de compra por indisponibilidade de insumo          | Nenhum pedido pode ser liberado para produção sem que a validação automática de estoque ou pedido de compra confirme a disponibilidade integral dos insumos calculados.    |
| RN012      | Reserva e baixa de insumos                                     | Na liberação, os insumos são reservados (indisponibilizados a outros pedidos); a baixa definitiva do saldo ocorre conforme o avanço das etapas produtivas.                 |
| RN013      | Sequência obrigatória de etapas                                | A situação do pedido evolui exclusivamente na sequência: Registrado → Validado → Liberado → Em produção → Finalizado → Expedido. Saltos de etapa são vedados.              |
| RN014      | Aprovação prévia da arte                                       | Pedidos cuja arte esteja na situação "Em aprovação" não podem ser liberados para produção; a aprovação deve ser registrada com data e responsável.                         |
| RN015      | Expiração de senha                                             | A senha dos usuários com perfil Administrador deve ser obrigatoriamente alterada a cada 90 dias.                                                                     |
| RN016      | Alteração de pedido                                            | O pedido é editável apenas até a liberação para produção. Após a liberação, alterações exigem autorização do Administrador e geram registro de justificativa na auditoria. |
| RN017      | Validação de documento do cliente                              | Todo cadastro de cliente pessoa física ou jurídica deve conter CPF ou CNPJ válido, com verificação dos dígitos verificadores e sem duplicidade na base.                    |
| RN018      | Vedação à exclusão física de registros                         | Pedidos, clientes e movimentações de estoque não são excluídos fisicamente; a inativação lógica preserva a rastreabilidade histórica.                                      |
| RN019      | Estoque mínimo                                                 | Materiais cujo saldo disponível atinja ou fique abaixo do estoque mínimo cadastrado são sinalizados ao perfil Estoque para ressuprimento.                                  |
| RN020      | Numeração do pedido                                            | Cada pedido recebe número sequencial único, gerado automaticamente pelo sistema e imutável ao longo de todo o ciclo de vida. |
| RN021      | Confirmação da entrada do pedido                               | A liberação para produção exige o registro da confirmação do pagamento de entrada, com data e usuário responsável. O sistema registra apenas a confirmação (sim/não), sem processar cobrança nem controlar valores. |

*Nota técnica: os índices das regras RN001 a RN009 devem ser mantidos como parâmetros configuráveis pelo Administrador, e não como valores fixos em código, de modo a absorver alterações de processo sem necessidade de nova versão do software. Recomenda-se que o consumo de tecido (RN001 e RN002) seja formalmente confirmado com a área de produção quanto à sua base de aplicação — por placa, conforme aqui adotado —, registrando-se a validação em ata de homologação.*
...
- RNF-DES-04: a emissão da ficha de produção em PDF não deve exceder 5 segundos.
...
- Impressora a laser ou jato de tinta (rede local): emissão da ficha de produção e dos relatórios operacionais em formato A4.
...
- Tablets de chão de fábrica: terminais de consulta da ficha de produção e de registro de status, conectados à rede local sem fio.
...
|------------------|------------------------------------------|---------------------------|--------------------------------|
| RF001            | RN015                                          | Autenticação / Auth       | CT-001, CT-002, CT-003         |
| RF002            | RN015                                          | Administração / RBAC      | CT-004, CT-005                 |
| RF003            | RN017, RN018                                   | Cadastro / Clientes       | CT-010, CT-011                 |
| RF005            | RN010, RN016, RN020                            | Pedidos                   | CT-020, CT-021, CT-022         |
| RF006            | RN001, RN002                                   | Pedidos                   | CT-023                         |
| RF007            | RN010                                          | Pedidos / Grade           | CT-024, CT-025                 |
| RF009            | RN014                                          | Pedidos / Arte            | CT-026                         |
| RF011            | RN005                                          | Pedidos / Embalagem       | CT-027                         |
| RF012            | RN001, RN002, RN003, RN004, RN006, RN007, RN008, RN009 | Cálculo de Insumos | CT-030, CT-031, CT-032, CT-033 |
| RF013            | RN011                                          | Estoque / Validação       | CT-040, CT-041, CT-042         |
| RF014            | RN012                                          | Estoque / Movimentação    | CT-043, CT-044                 |
| RF015            | RN011, RN013, RN014, RN021                     | Produção / Liberação      | CT-050, CT-053                 |
| RF016            | RN013                                          | Produção / Acompanhamento | CT-051, CT-052                 |
| RF019            | RN018                                          | Estoque / Movimentação    | CT-060, CT-061                 |
| RF020            | RN019                                          | Estoque / Alertas         | CT-062                         |
| RF023            | RN016, RN018                                   | Auditoria / Histórico     | CT-070, CT-071                 |
...
| Evento                                     | Tipo, data e observações do evento de destino.                          | 1:N com Pedido                                   |
...
| Item de Pedido / Distribuição de Numeração | Distribuição dos pares por gênero e numeração e cor.                    | N:1 com Pedido                                   |
| Produto / Qualidade                        | Classificação Normal (50/50) ou Premium (70/30) e respectivos índices.  | 1:N com Pedido                                   |
| Tira                                       | Genero, cor das tiras utilizadas.                                       | N:N com Pedido; N:1 com Material                 |
...
- Registro da confirmação da entrada do pedido pelo perfil Comercial / Atendimento.
- Confirmada a disponibilidade, aprovada a arte e registrada a entrada, o pedido é liberado para produção e os insumos são reservados.
...
| 1.1 | dd/mm/aaaa | Equipe de Análise | Incorporação das revisões de negócio: novas regras de consumo de tinta sublimática (RN008) e de tag e trava anel (RN009); criação da RN021 (confirmação da entrada); renumeração das regras RN010 a RN020 e atualização de todas as referências cruzadas e da matriz de rastreabilidade. |
